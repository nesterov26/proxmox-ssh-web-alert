...
. /opt/proxmox_scripts/credentials/credentials.config
for i in "${USERID[@]}"
do
...
More informations: [${IPINFO}](${IPINFO})"
        ...
        if [ -z "$1" ] || [ -z "$2" ]; then
                echo 'You need send parameter to script: $1-Ip, $2-User'
                exit 0
        fi
        CLIENT_IP=$1
        USER_NAME=$2
        ...
        TEXT="*Web-Connection* from *${CLIENT_IP}* as *${USER_NAME}* on *${SRV_HOSTNAME}* (*${SRV_IP}*)
Date: ${DATE}
More informations: [${IPINFO}](${IPINFO})"
